...
function main_non_periodic(m::Int, h::Real=1.0e-3; block_size::Int = div(m,2))
	...
	for beta in betas_list
		obs1 = interactionH(model, i, j, BoundaryMPS(D2=D2, D1=D1), β=beta)
		obs2 = interactionH(model, i, j, exp_alg, β=beta)
	...
	# data_path = "result/classical_non_periodic_tn_int_N$(m)_h$(h)_blocksize$(block_size)_D$(D1).json"
...
# function main_periodic(m::Int, h::Real=1.0e-3; block_size::Int = div(m,2))
# 	i = j = div(m, 2)
...
# 	for beta in betas_list
# 		obs1 = interactionH(model, i, j, BoundaryMPO(D2=D2, D1=D1), β=beta)
# 		obs2 = interactionH(model, i, j, exp_alg, β=beta)
# 		m_Onsager = Onsager_m(beta)
...
# 		println("beta=$beta, BoundaryMPO=$obs1, BP=$obs2, Onsager=$(m_Onsager)")
# 	end


# 	data_path = "result/classical_periodic_tn_int_N$(m)_h$(h)_blocksize$(block_size)_D$(D1).json"
# 	results = Dict("boundarympo"=>boundary_mps_results, "bp"=>bp_results, "Onsager"=>Onsager_results, "betas"=>betas_list)
# 	open(data_path, "w") do f
# 		write(f, JSON.json(results))
# 	end
