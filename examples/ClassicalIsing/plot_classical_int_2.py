...
i = 0
ax.plot(betas, boundarymps, color = colors[i], marker=markers[i], markersize=markersize,  ls='--',  linewidth=linewidth, markerfacecolor='none', label='BMPS, h=%s'%(h))

i += 1
ax.plot(betas, bp, color = colors[i], marker=markers[i], markersize=markersize,  ls='--',  linewidth=linewidth, markerfacecolor='none', label='BP, h=%s'%(h))
...
ax.set_ylabel(r'$m$', fontsize=fontsize, rotation=90)
...
plt.savefig('classical_tn_interaction_2.pdf', dpi=150)
