...
def gen_classical_tn_path(N, h, block_size, D, isperiodic):
	if isperiodic:
		return "result/random_classical_periodic_tn_N%s_h%s_blocksize%s_D%s.json"%(N, h, block_size, D)
	else:
		return "result/random_classical_non_periodic_tn_N%s_h%s_blocksize%s_D%s.json"%(N, h, block_size, D)
...
i = 0
ax.plot(betas, boundarymps, color = colors[i], marker=markers[i], markersize=markersize,  ls='--',  linewidth=linewidth, markerfacecolor='none', label='h=%s'%(h))
...
plt.savefig('classical_tn_check_random_2.pdf', dpi=150)
