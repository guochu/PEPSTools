...
def gen_classical_tn_path(N, h, block_size, D, isperiodic):
	if isperiodic:
		return "result/corner_classical_periodic_tn_N%s_h%s_blocksize%s_D%s.json"%(N, h, block_size, D)
	else:
		return "result/corner_classical_non_periodic_tn_N%s_h%s_blocksize%s_D%s.json"%(N, h, block_size, D)
...
plt.savefig('classical_tn_check_corner_2.pdf', dpi=150)
