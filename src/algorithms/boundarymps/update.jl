	...
	if n_nontrivial_terms(U.V) > 0
		...
		for i in 1:n
			...
			if n_nontrivial_terms(U.V[:, i]) > 0
				...
				update_bonds!(row_i, U.V[:, i], trunc=trunc, maxiter=als_maxiter, tol=als_tol, verbosity=als_verbosity, normalize=true)
